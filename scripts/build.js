...
function buildKaggleLesson(spec, lang) {
  ...
  const t = zh ? {
    ...
    warmup: "先预测，再运行", vocab: "今天要会用的两个词", mental: "两个核心概念",
    ...
    warmup: "Predict first, then run", vocab: "Two words to use today", mental: "Two core concepts",
  ...
  S.push(sec("title", `<div class="kicker" style="color:#7fd3df">${spec.classNo === "01" ? (zh ? "DEMO 试讲课 / PYTHON FOR AI RESEARCH" : "DEMO CLASS / PYTHON FOR AI RESEARCH") : `PYTHON FOR AI RESEARCH / CLASS ${spec.classNo}`}</div><h1 style="color:#fff">${esc(spec.title)}</h1><p style="color:#cfe0ef;margin-top:.45em">${esc(spec.subtitle)}</p><div class="grid2" style="margin-top:1em">${spec.skills.map((skill) => `<div class="skillcard"><div class="cn">${esc(skill)}</div><p>${esc(spec.stage)}</p></div>`).join("")}</div>${notes("35页、120分钟课堂版；新增页面用于练习、项目推进和证据检查。", "A 35-slide, 120-minute lesson with additional practice, project work, and evidence checks.")}`, "center", 'data-background-gradient="linear-gradient(135deg,#0C2D52,#16406e)"'));
  S.push(sec("outputs", `<div class="kicker">${t.output}</div><h2>${esc(spec.output)}</h2><div class="grid3" style="margin-top:.65em">${boxes([[t.understand, zh ? "能用自己的话解释两个核心概念。" : "Explain the two core ideas in your own words."], [t.practice, zh ? "运行并修改课堂示例。" : "Run and modify the lesson example."], [t.submit, zh ? "把成果保存到项目Notebook。" : "Save the result in the project notebook."]])}</div>${notes("先说明本节课唯一可见产出。", "Begin with the single visible lesson outcome.")}`));
  ...
  S.push(sec("one-twenty-minute-pacing", `<div class="kicker">${t.pacing}</div><h2>${t.pacingTitle}</h2><div class="flow" style="margin-top:.55em"><div class="step"><span class="n">0-10</span><b>Start</b><span>${zh ? "目标与热身" : "Goal and warm-up"}</span></div><div class="step"><span class="n">10-30</span><b>Concept</b><span>${zh ? "两个核心概念" : "Two core ideas"}</span></div><div class="step"><span class="n">30-50</span><b>Demo</b><span>${zh ? "代码演示与追踪" : "Demo and trace"}</span></div><div class="step"><span class="n">50-70</span><b>Practice</b><span>${zh ? "两轮跟做" : "Two guided rounds"}</span></div><div class="step"><span class="n">70-75</span><b>Break</b><span>${zh ? "短休息" : "Short break"}</span></div><div class="step"><span class="n">75-105</span><b>Project</b><span>${zh ? "20分钟独立操作 + 10分钟巡检反馈" : "20 min independent + 10 min review"}</span></div><div class="step loop"><span class="n">105-120</span><b>Close</b><span>${zh ? "证据、提交、退出条" : "Evidence, submit, exit"}</span></div></div>${notes("75–105分钟中约20分钟用于学生独立操作，约10分钟用于教师巡检、个别反馈和统一纠错。", "Within minutes 75–105, students work independently for about 20 minutes and the instructor uses about 10 minutes for check-ins, individual feedback, and a shared correction.")}`));
  if (spec.classNo === "01") {
    S.push(sec("prerequisite-recap", `<div class="kicker">${zh ? "DEMO 试讲课 · 从这里开始" : "DEMO CLASS · START HERE"}</div><h2>${zh ? "第一次见面：零基础也能完成一次真实的 Python 体验" : "First meeting: complete a real Python experience with no prior knowledge"}</h2><div class="grid2" style="margin-top:.65em"><div class="box"><h3>${zh ? "不需要提前学过" : "No prior experience required"}</h3><p>${zh ? "不要求学过 Python 或 Kaggle；只需要打开浏览器，跟着教师亲手运行和修改代码。" : "No Python or Kaggle background is required; open a browser and run and modify code with the instructor."}</p></div><div class="box"><h3>${zh ? "今天当堂带走" : "Leave with a real result"}</h3><p>${esc(spec.output)}</p></div></div><div class="callout"><b>${zh ? "体验方式：" : "How the demo works: "}</b>${zh ? "教师先示范，学生现场运行、修改并保存自己的成果；每一步都看得见、说得清。" : "The instructor demonstrates first; students then run, modify, and save their own work, with every step visible and explainable."}</div>${notes("这是第一节Demo试讲课。先降低进入门槛并建立信心，不使用复习或续课话术。", "This is the first demo class. Lower the entry barrier and build confidence; do not use recap language.")}`));
  } else {
  ...
  if (spec.classNo === "01") {
    S.push(sec("dataset-context", `<div class="kicker">GOOGLE COLAB · QUICK START</div><h2>${zh ? "四步打开你的第一份云端 Python Notebook" : "Open your first cloud Python notebook in four steps"}</h2><div class="flow" style="margin-top:.7em"><div class="step"><span class="n">1</span><b>${zh ? "打开" : "Open"}</b><span>colab.research.google.com</span></div><div class="step"><span class="n">2</span><b>${zh ? "登录" : "Sign in"}</b><span>${zh ? "使用Google账号" : "Use a Google account"}</span></div><div class="step"><span class="n">3</span><b>${zh ? "新建" : "Create"}</b><span>${zh ? "点击New notebook" : "Click New notebook"}</span></div><div class="step loop"><span class="n">4</span><b>${zh ? "运行" : "Run"}</b><span>print("Hello, AI!")</span></div></div><div class="callout"><b>${zh ? "为什么用Colab：" : "Why Colab: "}</b>${zh ? "无需安装、自动保存、方便分享，适合第一次接触Python的学生。" : "No installation, automatic saving, and easy sharing make it suitable for first-time Python learners."}</div>${notes("现场完成打开、登录、新建、运行四步；不要只讲界面。", "Complete open, sign in, create, and run live; do not merely describe the interface.")}`));
  } else {
  ...
  S.push(labSlide("guided-lab", "PYTHON LAB · BROWSER / PYODIDE", spec.labTitle, spec.labIntro, spec.labCode, spec.requiredPackages || [], zh ? "无文件的小练习可在浏览器运行；包依赖由页面明确加载。" : "This file-free micro-practice runs in the browser; required packages are declared by the slide."));
  ...
  S.push(sec("project-lab", `<div class="kicker">KAGGLE NOTEBOOK CODE · NOT A BROWSER LAB</div><h2>${esc(`${spec.projectTitle} · ${t.projectStarter}`)}</h2><p>${zh ? "这段代码需要学生的Kaggle项目文件或前序变量，只在Kaggle Notebook运行。" : "This code needs the student's Kaggle project files or earlier variables and runs only in the Kaggle notebook."}</p><pre style="font-size:.52em;max-height:390px;overflow:auto"><code>${esc(spec.projectCode)}</code></pre><div class="callout"><b>${zh ? "边界：" : "Boundary: "}</b>${zh ? "浏览器实验只承担无文件的小练习；完整项目、文件读取和项目输出留在Kaggle。" : "Browser labs are for small file-free practice; full projects, file loading, and project outputs stay in Kaggle."}</div>${notes("学生选择主题，教师控制技术骨架；不要点击浏览器Run按钮运行项目文件代码。", "Students choose the topic while the instructor controls the technical spine; project-file code is not presented with a browser Run button.")}`));
  ...
  if (spec.classNo === "01") {
    S.push(sec("rubric", `<div class="kicker">DEMO SUCCESS CHECK</div><h2>${zh ? "这节体验课成功，只看四个可见结果" : "A successful demo has four visible results"}</h2><div class="grid2" style="grid-template-columns:1fr 1fr;margin-top:.65em"><div class="box"><h3>Run</h3><p>${zh ? "亲手运行了至少一段Python代码。" : "Run at least one Python code block personally."}</p></div><div class="box"><h3>Change</h3><p>${zh ? "修改了一个变量并看到输出变化。" : "Change one variable and observe the output."}</p></div><div class="box"><h3>Explain</h3><p>${zh ? "能指出一项输入和一项输出。" : "Identify one input and one output."}</p></div><div class="box"><h3>Save</h3><p>${zh ? "保存了自己的Notebook副本。" : "Save a personal notebook copy."}</p></div></div>${notes("Demo课不强调分数，强调学生亲手完成的证据。", "The demo emphasizes evidence of doing, not grades.")}`));
    S.push(sec("homework", `<div class="kicker">KEEP YOUR RESULT</div><h2>${zh ? "Demo结束后，把今天的成果真正留在自己手里" : "Keep today's result after the demo"}</h2><div class="grid2" style="margin-top:.65em"><div class="box">${list(spec.homework)}</div><div class="box"><h3>${zh ? "离开前确认" : "Before leaving"}</h3>${list(zh ? ["代码从上到下可运行", "四种类型至少出现三种", "有一句自己的修改", "Notebook链接可以访问"] : ["Code runs top to bottom", "At least three of the four types appear", "One change is personal", "The notebook link is accessible"])}</div></div>${notes("不布置新的大任务，只确保学生带走可运行成果。", "Do not assign a new large task; make sure the student leaves with a runnable artifact.")}`));
  } else {
