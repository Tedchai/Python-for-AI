...
const KAGGLE_COURSE = require("./kaggle-course");
...
// The current course uses a 120-minute, Kaggle-centered 3+1 stage rhythm.
// Keep the earlier catalog literal as historical source material, then replace
// the live catalog with the aligned bilingual course data.
const chineseCourse = CATALOG.courses.find((course) => course.id === "python-ai");
chineseCourse.subtitle = "以 Kaggle 真实数据项目贯穿 Python、数据分析、基础机器学习与 Draft Paper 的 15 节高中生课程。";
chineseCourse.tags = ["Python", "Kaggle", "Draft Paper", "High School"];
chineseCourse.decks = KAGGLE_COURSE.catalog.zh;
englishCourse.subtitle = "A 15-class high-school pathway from Kaggle data and Python skills to a credible Draft Paper.";
englishCourse.tags = ["Python", "Kaggle", "Draft Paper", "English"];
englishCourse.decks = KAGGLE_COURSE.catalog.en;
...
.reveal .step .n.time{width:auto;min-width:2.6em;padding:0 .35em;border-radius:999px;white-space:nowrap}
...
function updateListen(){
  ...
  const has=currentSlide?.dataset.narration!=='off' && !!_narrationData[curSid];
...
for (const no of Object.keys(syllabusMap)) {
  Object.assign(syncedZhLessons[no], {
    duration: ["01", "02"].includes(no) ? 120 : 60, stage: stageForLesson(no, "zh"), requiredPackages: packagesForLesson[no],
...
function buildKaggleLesson(spec, lang) {
  const zh = lang === "zh";
  const liveEnglishClassOne = !zh && spec.classNo === "01";
  const S = [];
  const pathwayReturn = liveEnglishClassOne
    ? '<p style="margin-top:.5em;font-size:.52em"><a href="#/prerequisite-recap" style="color:var(--teal);font-weight:800;text-decoration:none">← Back to course pathways</a></p>'
    : "";
  const notes = (zhText, enText) => `<aside class="notes">${esc(zh ? zhText : enText)}</aside>`;
  const list = (items) => `<ul>${items.map((item) => `<li>${esc(item)}</li>`).join("")}</ul>`;
  const boxes = (items) => items.map(([heading, body], i) => `<div class="box"><span class="feature"${i === 1 ? ' style="background:var(--green)"' : i === 2 ? ' style="background:var(--amber)"' : ""}>${i + 1}</span><h3>${esc(heading)}</h3><p>${esc(body)}</p></div>`).join("");
  const t = zh ? {
    output: "TODAY'S OUTPUT", understand: "理解", practice: "练习", submit: "提交",
    pacing: "120-MINUTE PACING", pacingTitle: "讲解、跟做、项目应用、论文证据",
    warmup: "先预测，再运行", vocab: "今天要会用的三个词", mental: "三个核心概念",
    syntax: "今天的Python模式", demo: "教师演示：完整运行一次", trace: "运行前先找输入、处理和输出",
    lab: "跟做练习", mistakes: "常见错误与修复", checkpoint: "独立检查点",
    project: "KAGGLE PROJECT CONNECTION", projectStarter: "项目Starter",
    evidence: "从Notebook到Draft Paper", rubric: "本节课评价标准", homework: "作业与提交检查",
    exit: "EXIT TICKET", run: "能运行", explain: "能解释", evidenceWord: "有证据", clean: "结构清楚",
  } : {
    output: "TODAY'S OUTPUT", understand: "Understand", practice: "Practice", submit: "Submit",
    pacing: "120-MINUTE PACING", pacingTitle: "Explain, practice, apply, and preserve evidence",
    warmup: "Predict first, then run", vocab: "Three words to use today", mental: "Three core concepts",
    syntax: "Today's Python pattern", demo: "Instructor demo: run the full workflow", trace: "Find the input, process, and output before running",
    lab: "Guided practice", mistakes: "Common errors and fixes", checkpoint: "Independent checkpoint",
    project: "KAGGLE PROJECT CONNECTION", projectStarter: "Project starter",
    evidence: "From Notebook to Draft Paper", rubric: "Lesson assessment", homework: "Homework and submission check",
  ...
  if (spec.classNo === "01") {
    S.push(sec("prerequisite-recap", `<div class="kicker">${zh ? "第一课 · 从这里开始" : "CLASS 01 · EXPLORE THE POSSIBILITIES"}</div><h2>${zh ? "第一次见面：零基础也能完成一份真实的 Python 成果" : "Where Python can take a young researcher"}</h2><div class="grid2" style="margin-top:.45em"><div class="box"><h3>${zh ? "轻松开始" : "Start comfortably"}</h3><p>${zh ? "不要求学过 Python 或 Kaggle；第一份代码直接在浏览器运行。" : "No prior Python or Kaggle experience is needed; the first result runs directly in the browser."}</p></div><div class="box"><h3>${zh ? "留下可见成果" : "Leave with visible progress"}</h3><p>${esc(spec.output)}</p></div></div>${!zh ? `<div class="flow" style="margin-top:.5em"><a class="step" href="#/vocabulary" style="color:inherit;text-decoration:none"><span class="n">1</span><b>Creative coding</b><span>Turn rules and choices into an interactive experience</span></a><a class="step" href="#/mental-model" style="color:inherit;text-decoration:none"><span class="n">2</span><b>Data stories</b><span>Use charts to reveal patterns and support a claim</span></a><a class="step" href="#/concept-3" style="color:inherit;text-decoration:none"><span class="n">3</span><b>AI experiments</b><span>Compare results and explain what the evidence supports</span></a><a class="step loop" href="#/course-position" style="color:inherit;text-decoration:none"><span class="n">4</span><b>Research poster</b><span>Bring the question, method, evidence, and limits into one story</span></a></div>` : ""}<div class="callout" style="margin-top:.5em"><b>${zh ? "使用方式：" : "Choose what fits: "}</b>${zh ? "四个方向可选择性预览；60分钟主线仍以亲手运行、修改和解释代码为核心。" : "Open any pathway for a short preview, or continue to the 60-minute core lesson on the next slide."}</div>${notes("第4页面向学生和家长介绍可能性；四个卡片链接到后续展开页，并可返回本页。", "Present four family-friendly possibilities. Each card opens a short preview with a link back to this course map.")}`));
  } else {
    S.push(sec("prerequisite-recap", `<div class="kicker">RECAP</div><h2>${zh ? "开始前只复习项目需要的内容" : "Review only what the project needs"}</h2><div class="grid2" style="margin-top:.65em"><div class="box"><h3>${zh ? "上节课留下了什么" : "What the previous class produced"}</h3><p>${esc(spec.stage)}</p></div><div class="box"><h3>${zh ? "今天将它推进到哪里" : "Where today moves it"}</h3><p>${esc(spec.output)}</p></div></div>${notes("复习控制在5分钟，只解决本节前置缺口。", "Keep recap to five minutes and address only prerequisite gaps.")}`));
  }
  S.push(sec("course-position", liveEnglishClassOne
    ? `<div class="kicker">PATHWAY PREVIEW · RESEARCH POSTER</div><h2>Turn many small results into one clear research story</h2><div class="grid2" style="margin-top:.65em"><div class="box"><h3>Evidence trail</h3><p>Keep the question, code, output, and limitation together so every poster claim can be traced to the notebook.</p></div><div class="box"><h3>Professional communication</h3><p>Organize the question, method, evidence, and limits in an IEEE-style visual structure.</p></div></div><div class="callout"><b>Course destination:</b> a research-communication milestone—not a promise of conference submission, acceptance, publication, or an award.</div>${pathwayReturn}${notes("说明Poster由可追溯证据逐步形成，不作投稿或录用承诺。", "Show how notebook evidence becomes a professional poster while avoiding submission or acceptance promises.")}`
  ...
  S.push(sec("warmup", liveEnglishClassOne
    ? `<div class="kicker">WARM-UP</div><h2>Predict first, then run</h2><div class="grid2" style="margin-top:.65em"><div class="box"><h3>Before running</h3>${list(["Predict one line of output", "Find one name or value that is unclear", "Choose one value to change"])}</div><div class="box"><h3>After running</h3>${list(["Compare the result with the prediction", "Explain why the change affected the output", "Keep the result as evidence"])}</div></div><div class="callout"><b>Why this matters:</b> prediction → test → explanation is the same evidence habit used in creative coding, data stories, AI experiments, and research posters.</div>${notes("暖场不使用角色分工标签，直接呈现可操作的思考流程。", "Present the thinking routine directly, without student-versus-instructor role labels.")}`
    : `<div class="kicker">WARM-UP</div><h2>${t.warmup}</h2><div class="grid2" style="margin-top:.65em"><div class="box"><h3>${zh ? "学生" : "Student"}</h3>${list(zh ? ["预测输出", "圈出不懂的变量", "只修改一个值"] : ["Predict the output", "Identify one unclear variable", "Change only one value"])}</div><div class="box"><h3>${zh ? "教师" : "Instructor"}</h3>${list(zh ? ["只追问一个为什么", "展示一次完整运行", "检查项目连接"] : ["Ask one why question", "Show one complete run", "Check the project connection"])}</div></div>${notes("暖场不超过5分钟。", "Keep the warm-up under five minutes.")}`));
  S.push(sec("vocabulary", liveEnglishClassOne
    ? `<div class="kicker">PATHWAY PREVIEW · CREATIVE CODING</div><h2>Small rules can create an interactive experience</h2><div class="grid2" style="margin-top:.65em"><div class="box"><h3>Python building blocks</h3>${list(["Variables remember the current state", "Types keep values meaningful", "Comments explain the design choice"])}</div><div class="box"><h3>Example idea</h3><p>A short decision game can respond to a choice, update a score, and explain why the result changed.</p><p><b>Course role:</b> one approachable example—not a separate game-design track.</p></div></div>${pathwayReturn}${notes("用已有变量、类型和注释概念解释创意编程，不把课程改成游戏设计方向。", "Use variables, types, and comments to preview creative coding without turning it into a separate game-design track.")}`
    : `<div class="kicker">VOCABULARY</div><h2>${t.vocab}</h2><div class="grid2" style="margin-top:.65em">${boxes(spec.vocab)}</div>${notes("术语只保留本节项目需要的两个。", "Keep only the two terms needed for the project.")}`));
  S.push(sec("mental-model", liveEnglishClassOne
    ? `<div class="kicker">PATHWAY PREVIEW · DATA STORIES</div><h2>Move from values to a chart that supports a claim</h2><div class="flow" style="margin-top:.75em"><div class="step"><span class="n">1</span><b>Store</b><span>Variables keep observations</span></div><div class="step"><span class="n">2</span><b>Check</b><span>Types prevent confusing comparisons</span></div><div class="step"><span class="n">3</span><b>Show</b><span>Output makes patterns visible</span></div><div class="step loop"><span class="n">4</span><b>Explain</b><span>A chart supports one careful conclusion</span></div></div><div class="callout"><b>Family-visible milestone:</b> a small set of clear charts, each connected to a question and an honest interpretation.</div>${pathwayReturn}${notes("把变量、类型和输出连接到数据故事与图表证据。", "Connect variables, types, and output to charts and evidence-based interpretation.")}`
    : `<div class="kicker">MENTAL MODEL</div><h2>${t.mental}</h2><div class="grid2" style="margin-top:.65em">${boxes(spec.concepts)}</div>${notes("先讲用途和直觉，再讲语法。", "Explain purpose and intuition before syntax.")}`));
  spec.concepts.forEach(([name, description], index) => {
    if (liveEnglishClassOne && index === 2) {
      S.push(sec("concept-3", `<div class="kicker">PATHWAY PREVIEW · AI EXPERIMENTS</div><h2>Compare results before making a model claim</h2><div class="grid3" style="margin-top:.65em">${boxes([["Question", "What should the model predict or classify?"], ["Fair comparison", "Use the same data split and metric for each model."], ["Evidence", "Report the result, error pattern, and limitation together."]])}</div><div class="callout"><b>Today's connection:</b> clear Python output is the first step toward trustworthy model evidence.</div>${pathwayReturn}${notes("用输出概念预览公平模型比较，强调证据与限制。", "Use the idea of output to preview fair model comparison, evidence, and limitations.")}`));
    } else {
      S.push(sec(`concept-${index + 1}`, `<div class="kicker">CONCEPT ${index + 1}</div><h2>${esc(name)}</h2><p class="lead">${esc(description)}</p><div class="callout"><b>${zh ? "一句话检查：" : "One-sentence check: "}</b>${zh ? "这个概念在我的项目里解决什么问题？" : "What problem does this idea solve in my project?"}</div>${notes("请学生用自己的项目字段回答。", "Ask the student to answer using a field from the project.")}`));
    }
  });
  ...
  S.push(sec("guided-practice-2", `<div class="kicker">GUIDED PRACTICE 2</div><h2>${zh ? "换成自己的项目字段" : "Switch to a field from the student's project"}</h2><div class="grid2" style="margin-top:.65em"><div class="box"><h3>${zh ? "代码修改" : "Code change"}</h3><p>${zh ? "把示例字段替换成项目字段，但保持代码骨架不变。" : "Replace the sample field while keeping the code structure unchanged."}</p></div><div class="box"><h3>${zh ? "口头解释" : "Oral explanation"}</h3><p>${zh ? "说明改了什么、输出怎样变化、为什么与研究问题有关。" : "Explain what changed, how the output changed, and why it matters to the question."}</p></div></div>${notes("第二轮学生独立完成，教师巡检。", "Students complete the second round independently with instructor check-ins.")}`));
  S.push(sec("guided-lab", `<div class="kicker">KAGGLE NOTEBOOK · GUIDED LAB</div><h2>${esc(spec.labTitle)}</h2><p class="lead">${esc(spec.labIntro)}</p><div class="grid2" style="grid-template-columns:.72fr 1.28fr;margin-top:.5em"><div class="box"><h3>${zh ? "学生操作" : "Student actions"}</h3>${list(zh ? ["在课程Notebook新增Code Cell", "粘贴并先预测一行输出", "运行后只修改一个因素", "重新运行并保存版本"] : ["Add a code cell in the course notebook", "Paste the code and predict one output", "Run, then change one factor", "Rerun and save a version"])}</div><pre style="font-size:.47em;max-height:350px;overflow:auto"><code>${esc(spec.labCode)}</code></pre></div><div class="callout"><b>${zh ? "课堂环境：" : "Class environment: "}</b>${zh ? "本练习的运行、报错、修改和保存全部在Kaggle Notebook完成。" : "Run, debug, revise, and save this practice entirely in Kaggle Notebook."}</div>${notes("展示任务后切换到Kaggle；课件不提供内置Run按钮。", "Switch to Kaggle after framing the task; the slide has no embedded Run button.")}`));
  S.push(sec("mistake-clinic", `<div class="kicker">MISTAKE CLINIC</div><h2>${t.mistakes}</h2><div class="grid3" style="margin-top:.65em">${boxes(spec.mistakes)}</div>${notes("只处理最可能阻塞本节产出的错误。", "Address only errors that block the lesson outcome.")}`));
  S.push(sec("debug-routine", `<div class="kicker">DEBUG ROUTINE</div><h2>${zh ? "读错误、找行号、缩小范围、重新运行" : "Read, locate, isolate, rerun"}</h2><div class="flow" style="margin-top:.75em"><div class="step"><span class="n">1</span><b>Read</b><span>${zh ? "读最后一行" : "Read the final line"}</span></div><div class="step"><span class="n">2</span><b>Locate</b><span>${zh ? "找到行号" : "Find the line"}</span></div><div class="step"><span class="n">3</span><b>Isolate</b><span>${zh ? "运行最小片段" : "Run the smallest block"}</span></div><div class="step loop"><span class="n">4</span><b>Rerun</b><span>${zh ? "从上到下重跑" : "Rerun top to bottom"}</span></div></div>${notes("每节课重复同一调试流程。", "Repeat the same debugging routine every class.")}`));
  S.push(sec("checkpoint", `<div class="kicker">CHECKPOINT</div><h2>${t.checkpoint}</h2><div class="grid3" style="margin-top:.65em">${boxes([[zh ? "Run" : "Run", zh ? "从上到下运行本节代码。" : "Run this lesson's code from top to bottom."], [zh ? "Explain" : "Explain", zh ? "指出输入、处理和输出。" : "Identify input, process, and output."], [zh ? "Connect" : "Connect", zh ? "说明它怎样服务自己的选题。" : "Explain how it supports the selected topic."]])}</div>${notes("五分钟巡检；需要帮助时粘贴最后一行错误。", "Five-minute check; students paste the final error line when blocked.")}`));
  S.push(sec("project-plan", `<div class="kicker">PROJECT PLAN</div><h2>${zh ? "开始项目前先写四行计划" : "Write a four-line plan before project work"}</h2><div class="grid2" style="margin-top:.65em"><div class="box">${list(zh ? ["我要使用哪个字段？", "我要生成什么输出？", "输出如何进入Notebook？", "它将支持论文哪一部分？"] : ["Which field will I use?", "What output will I produce?", "Where will it go in the notebook?", "Which paper section will it support?"])}</div><div class="callout"><b>${zh ? "时间边界：" : "Time boundary: "}</b>${zh ? "只完成本节定义的项目动作，不临时增加新模型。" : "Complete only today's defined project action; do not add a new model."}</div></div>${notes("先计划再编码，帮助教师控制五个不同选题。", "Plan before coding so the instructor can manage five different topics.")}`));
  S.push(sec("project-brief", `<div class="kicker">${t.project}</div><h2>${esc(spec.projectTitle)}</h2><div class="grid3" style="margin-top:.65em">${boxes([["Input", spec.projectInput], ["Process", spec.projectProcess], ["Output", spec.projectOutput]])}</div>${notes("项目任务必须能在本节剩余时间内启动。", "The project task must be startable within the lesson.")}`));
  S.push(sec("project-lab", `<div class="kicker">KAGGLE NOTEBOOK · PROJECT LAB</div><h2>${esc(`${spec.projectTitle} · ${t.projectStarter}`)}</h2><p>${zh ? "在同一份Kaggle Notebook继续使用前序变量，把课堂练习推进为可保存的项目证据。" : "Continue from earlier variables in the same Kaggle Notebook and turn the practice into saved project evidence."}</p><pre style="font-size:.52em;max-height:390px;overflow:auto"><code>${esc(spec.projectCode)}</code></pre><div class="callout"><b>${zh ? "提交边界：" : "Submission boundary: "}</b>${zh ? "只提交亲手运行、能够解释并已Save Version的Notebook结果。" : "Submit only notebook results that were personally run, can be explained, and were saved as a version."}</div>${notes("学生在Kaggle完成项目代码；教师巡检输入、处理、输出和保存状态。", "Students complete the project code in Kaggle; check input, process, output, and saved-version status.")}`));
  S.push(sec("independent-work", `<div class="kicker">INDEPENDENT WORK</div><h2>${zh ? "20分钟项目工作室" : "20-minute project studio"}</h2><div class="grid3" style="margin-top:.65em">${boxes([[zh ? "前7分钟" : "First 7 min", zh ? "复制Starter并替换字段。" : "Copy the starter and replace fields."], [zh ? "中间8分钟" : "Next 8 min", zh ? "运行、调试并保存输出。" : "Run, debug, and save output."], [zh ? "最后5分钟" : "Final 5 min", zh ? "写Markdown解释并检查证据。" : "Write a Markdown explanation and check evidence."]])}</div>${notes("教师按学生名单逐个巡检，不同时引入新技术。", "Check students one by one without introducing new techniques.")}`));
  S.push(sec("challenge-ladder", `<div class="kicker">CHALLENGE LADDER</div><h2>${zh ? "所有人完成Level 1，进阶只加一个小扩展" : "Everyone completes Level 1; advanced students add one small extension"}</h2><div class="flow" style="margin-top:.75em"><div class="step"><span class="n">1</span><b>Core</b><span>${zh ? "代码运行并产生要求输出" : "Run and produce the required output"}</span></div><div class="step"><span class="n">2</span><b>Explain</b><span>${zh ? "修改一个字段并解释" : "Change one field and explain"}</span></div><div class="step loop"><span class="n">3</span><b>Extend</b><span>${zh ? "增加一个图、检查或比较" : "Add one figure, check, or comparison"}</span></div></div>${notes("Level 3不允许改变项目任务类型。", "Level 3 must not change the project task type.")}`));
  S.push(quizSlide("concept-check", "CONCEPT CHECK", spec.quiz2[0], spec.quiz2[1], spec.quiz2[2], spec.quiz2[3], zh ? "用第二题检查项目应用是否理解。" : "Use the second question to check project understanding."));
  S.push(sec("evidence-handoff", `<div class="kicker">${t.evidence}</div><h2>${zh ? "今天的代码将进入哪一部分？" : "Where will today's code appear?"}</h2><div class="grid2" style="margin-top:.65em"><div class="box"><h3>Kaggle Notebook</h3><p>${esc(spec.kaggleAction)}</p></div><div class="box"><h3>Draft Paper</h3><p>${esc(spec.paperSection)}</p></div></div><div class="callout"><b>${zh ? "底线：" : "Minimum standard: "}</b>${zh ? "论文中的方法、数字和图表必须能回到Notebook找到来源。" : "Every method, number, and figure in the paper must trace back to the notebook."}</div>${notes("持续建立可追溯证据链。", "Build a traceable evidence chain throughout the course.")}`));
  S.push(sec("notebook-hygiene", `<div class="kicker">NOTEBOOK HYGIENE</div><h2>${zh ? "提交前整理代码、输出和说明" : "Clean code, output, and notes before submission"}</h2><div class="grid2" style="margin-top:.65em"><div class="box"><h3>Code</h3>${list(zh ? ["从上到下能运行", "变量名可读", "删除无关草稿", "保留必要输出"] : ["Runs top to bottom", "Readable names", "Remove unrelated drafts", "Keep necessary output"])}</div><div class="box"><h3>Markdown</h3>${list(zh ? ["说明输入和输出", "记录自己的修改", "解释项目意义", "写一个限制"] : ["Explain input and output", "Record the student's change", "Explain project relevance", "State one limitation"])}</div></div>${notes("预留整理时间，避免交付无法复现的Notebook。", "Reserve cleanup time to avoid an irreproducible notebook.")}`));
  S.push(sec("research-integrity", `<div class="kicker">AI & RESEARCH INTEGRITY</div><h2>${zh ? "可以用AI获得提示，但不能提交自己无法解释的代码" : "AI may provide hints, but students cannot submit code they cannot explain"}</h2><div class="grid2" style="margin-top:.65em"><div class="box"><h3>${zh ? "可以" : "Allowed"}</h3>${list(zh ? ["解释报错", "生成相似练习", "检查语句是否清楚", "提供修改提示"] : ["Explain an error", "Generate similar practice", "Check clarity", "Offer revision hints"])}</div><div class="box"><h3>${zh ? "不可以" : "Not allowed"}</h3>${list(zh ? ["代写完整项目", "伪造结果", "隐藏数据来源", "提交无法解释的模型"] : ["Write the full project", "Fabricate results", "Hide the data source", "Submit an unexplained model"])}</div></div>${notes("Stage检查时随机抽取代码让学生口头解释。", "During Stage reviews, ask students to explain selected code aloud.")}`));
  if (spec.classNo === "01") {
    S.push(sec("rubric", `<div class="kicker">TODAY'S SUCCESS CHECK</div><h2>${zh ? "完成第一课，要留下四个可见结果" : "Complete Class 01 with four visible results"}</h2><div class="grid2" style="grid-template-columns:1fr 1fr;margin-top:.55em"><div class="box"><h3>Run</h3><p>${zh ? "亲手运行了至少一段Python代码。" : "Run at least one Python code block personally."}</p></div><div class="box"><h3>Change</h3><p>${zh ? "修改了一个变量并看到输出变化。" : "Change one variable and observe the output."}</p></div><div class="box"><h3>Explain</h3><p>${zh ? "能指出一项输入和一项输出。" : "Identify one input and one output."}</p></div><div class="box"><h3>Save</h3><p>${zh ? "保存了自己的Notebook副本。" : "Save a personal notebook copy."}</p></div></div>${!zh ? `<div class="callout" style="margin-top:.55em"><b>FIRST FAMILY-VISIBLE MILESTONE:</b> a runnable notebook the student can explain in their own words.</div>` : ""}${notes("第一课不强调分数，强调学生亲手完成的证据。", "Class 01 emphasizes evidence of doing rather than grades. The family-facing proof is a runnable notebook the student can explain.")}`));
    S.push(sec("homework", `<div class="kicker">KEEP YOUR RESULT</div><h2>${zh ? "下课前，把今天的成果真正留在自己手里" : "Keep today's result before class ends"}</h2><div class="grid2" style="grid-template-columns:.9fr 1.1fr;margin-top:.55em"><div class="box">${list(spec.homework)}</div><div class="box"><h3>${zh ? "离开前确认" : "Visible progress checkpoints"}</h3>${list(zh ? ["代码从上到下可运行", "四种类型至少出现三种", "有一句自己的修改", "Notebook链接可以访问"] : ["Class 1–3 · Runnable Python notebook", "Class 4–8 · Data story with clear charts", "Class 9–13 · Model evidence and comparison", "Class 14–15 · IEEE-style research poster"])}</div></div>${!zh ? `<div class="callout" style="margin-top:.5em"><b>Today:</b> keep the code, output, and explanation that will later support a poster claim.</div>` : ""}${notes("不布置新的大任务，只确保学生带走可运行成果。", "Homework extends today's work. Use the four checkpoints to show families concrete progress without promising a conference submission or acceptance.")}`));
  } else {
    S.push(sec("rubric", `<div class="kicker">RUBRIC</div><h2>${t.rubric}</h2><div class="grid2" style="grid-template-columns:1fr 1fr;margin-top:.65em"><div class="box"><h3>35% ${t.run}</h3><p>${zh ? "代码可从上到下运行。" : "Code runs from top to bottom."}</p></div><div class="box"><h3>30% ${t.explain}</h3><p>${zh ? "能说明关键代码和选择。" : "Key code and choices are explained."}</p></div><div class="box"><h3>25% ${t.evidenceWord}</h3><p>${zh ? "输出服务于项目或论文。" : "The output supports the project or paper."}</p></div><div class="box"><h3>10% ${t.clean}</h3><p>${zh ? "Notebook结构和命名清楚。" : "Notebook structure and naming are clear."}</p></div></div>${notes("不以复杂模型或代码数量加分。", "Do not reward model complexity or code volume.")}`));
    S.push(sec("homework", `<div class="kicker">HOMEWORK</div><h2>${t.homework}</h2><div class="grid2" style="margin-top:.65em"><div class="box">${list(spec.homework)}</div><div class="box"><h3>${zh ? "提交前" : "Before submission"}</h3>${list(zh ? ["全部cells可运行", "Notebook链接可访问", "数据来源与许可已记录", "能口头解释关键代码"] : ["All cells run", "Notebook link is accessible", "Data source and license are recorded", "Key code can be explained aloud"])}</div></div>${notes("作业只延续课堂已经启动的任务。", "Homework extends only work already started in class.")}`));
  }
  S.push(sec("buffer-catchup", `<div class="kicker">BUFFER / CATCH-UP</div><h2>${zh ? "最后5分钟按进度分流" : "Use the final five minutes by progress level"}</h2><div class="grid3" style="margin-top:.65em">${boxes([[zh ? "仍有错误" : "Still blocked", zh ? "提交最后一行错误并保存当前版本。" : "Submit the final error line and save the current version."], [zh ? "刚刚完成" : "Just finished", zh ? "口述30秒代码解释。" : "Give a 30-second code explanation."], [zh ? "提前完成" : "Finished early", zh ? "完成Level 3并补写限制。" : "Complete Level 3 and add a limitation."]])}</div>${notes("缓冲页确保120分钟课堂能够统一收尾。", "This buffer keeps the 120-minute lesson on a shared close.")}`));
  S.push(sec("exit-ticket", `<div class="kicker">${t.exit}</div><h2>${esc(spec.line)}</h2><div class="grid2" style="margin-top:.65em"><div class="box"><h3>${zh ? "我现在能做什么？" : "What can I do now?"}</h3><p>${zh ? "用一句话说明本节新增的Python能力。" : "State the new Python capability in one sentence."}</p></div><div class="box"><h3>${zh ? "项目下一步是什么？" : "What is the project next step?"}</h3><p>${esc(spec.kaggleAction)}</p></div></div>${notes("收集一个清楚点和一个仍需帮助的问题。", "Collect one clear point and one remaining question.")}`, "center", 'data-background-gradient="linear-gradient(135deg,#EEF4FA,#FFFFFF)"'));

  // Stage lessons use a review-conference template. They preserve the same 35
  // cross-language slide IDs while replacing concept repetition with student
  // explanation, teacher feedback, revision records, and explicit gate checks.
  if (spec.lessonType === "stage" && spec.stageReview) {
    const review = spec.stageReview;
    const stageLabel = `STAGE ${review.number} / 4`;
    const stageSlide = (id, kicker, title, body, noteZh, noteEn) => sec(id, `<div class="kicker">${esc(kicker)}</div><h2>${esc(title)}</h2>${body}${notes(noteZh, noteEn)}`);
    const revisions = zh
      ? ["学生先口头解释当前版本", "教师只给一项最高优先级反馈", "学生修改并记录前后差异"]
      : ["Student explains the current version", "Instructor gives one highest-priority item", "Student revises and records the before/after change"];
    const overrides = {
      "prerequisite-recap": stageSlide("prerequisite-recap", `${stageLabel} · CURRENT MATERIALS`, zh ? "先摆出本阶段已有材料" : "Start with the current stage materials", `<div class="grid2" style="margin-top:.65em"><div class="box">${list(review.materials)}</div><div class="callout"><b>${zh ? "缺失项：" : "Missing item: "}</b>${zh ? "若材料不全，先标记并安排补交，不用新概念掩盖缺口。" : "If an item is missing, flag and schedule it; do not hide the gap with a new concept."}</div></div>`, "逐项点名材料，不重新讲授知识课。", "Name each artifact; do not reteach a knowledge lesson."),
      "dataset-context": stageSlide("dataset-context", `${stageLabel} · REVIEW CRITERIA`, zh ? "通过条件必须可观察" : "Gate criteria must be observable", `<div class="grid2" style="margin-top:.65em">${review.criteria.map((item, i) => `<div class="box"><span class="feature">${i + 1}</span><p>${esc(item)}</p></div>`).join("")}</div>`, "评价具体产出，不评价代码数量。", "Evaluate concrete evidence, not code volume."),
      warmup: stageSlide("warmup", `${stageLabel} · STATUS CHECK`, zh ? "60秒学生状态汇报" : "60-second student status report", `<div class="grid3" style="margin-top:.65em">${boxes([[zh ? "已完成" : "Done", zh ? "展示一个可以运行或阅读的成果。" : "Show one runnable or readable artifact."], [zh ? "卡点" : "Blocker", zh ? "指出具体文件、字段或代码行。" : "Name the exact file, field, or code line."], [zh ? "本节目标" : "Today's target", zh ? "选择一个必须完成的修订。" : "Choose one revision that must be completed."]])}</div>`, "先让学生自己判断项目状态。", "Let the student assess project status first."),
      vocabulary: stageSlide("vocabulary", `${stageLabel} · DELIVERABLE LANGUAGE`, zh ? "本阶段只保留四个交付词" : "Use four stage-deliverable terms", `<div class="grid2" style="margin-top:.65em">${review.materials.slice(0, 4).map((item) => `<div class="box"><h3>${esc(item)}</h3><p>${zh ? "学生能指出它在哪里、如何生成、还要修改什么。" : "The student can locate it, explain how it was produced, and name the needed revision."}</p></div>`).join("")}</div>`, "术语服务于交付检查。", "Terms support the artifact review."),
      "mental-model": stageSlide("mental-model", `${stageLabel} · STAGE GOAL`, zh ? "本阶段的唯一通过目标" : "The single gate goal", `<p class="lead">${esc(review.goal)}</p><div class="callout"><b>${zh ? "判断原则：" : "Decision rule: "}</b>${zh ? "证据不足时标记需要修改，不用“差不多”代替检查。" : "When evidence is incomplete, mark a revision; do not replace review with 'close enough.'"}</div>`, "用目标控制检查范围。", "Use the goal to control review scope."),
      "concept-1": stageSlide("concept-1", `${stageLabel} · STUDENT CODE EXPLANATION`, zh ? "学生解释一段关键代码" : "Student explains one key code block", `<pre style="font-size:.5em;max-height:300px;overflow:auto"><code>${esc(spec.projectCode)}</code></pre><div class="grid3" style="margin-top:.45em">${boxes([["Input", zh ? "输入来自哪个文件或变量？" : "Which file or variable provides the input?"], ["Process", zh ? "代码改变或计算了什么？" : "What does the code change or calculate?"], ["Output", zh ? "输出保存在哪里、支持哪条证据？" : "Where is the output saved, and which evidence does it support?"]])}</div>`, "教师随机指定代码行，学生不能只朗读。", "The instructor selects a line; the student must explain rather than read it aloud."),
      "concept-2": stageSlide("concept-2", `${stageLabel} · TEACHER QUESTIONS`, zh ? "教师只问能够改变下一步的问题" : "Ask questions that change the next step", `<div class="grid3" style="margin-top:.65em">${review.questions.map((item, i) => `<div class="box"><span class="feature">Q${i + 1}</span><p>${esc(item)}</p></div>`).join("")}</div>`, "记录学生回答和需要补充的证据。", "Record the answer and any missing evidence."),
      "syntax-pattern": stageSlide("syntax-pattern", `${stageLabel} · EVIDENCE AUDIT`, zh ? "把代码、输出和文字一一对齐" : "Align code, output, and prose", `<div class="flow" style="margin-top:.75em"><div class="step"><span class="n">1</span><b>Code</b><span>${zh ? "找到生成结果的单元格" : "Find the cell that creates the result"}</span></div><div class="step"><span class="n">2</span><b>Output</b><span>${zh ? "核对数字、表格或图" : "Verify the number, table, or figure"}</span></div><div class="step loop"><span class="n">3</span><b>Claim</b><span>${zh ? "检查文字没有超出证据" : "Check that prose stays within evidence"}</span></div></div>`, "Stage课不新增复杂语法。", "Stage lessons do not introduce complex syntax."),
      "teacher-demo": stageSlide("teacher-demo", `${stageLabel} · REVIEW CONFERENCE`, zh ? "教师示范一次两分钟检查" : "Instructor models a two-minute review", `<div class="grid3" style="margin-top:.65em">${boxes([[zh ? "展示" : "Show", zh ? "学生打开当前成果。" : "Student opens the current artifact."], [zh ? "追问" : "Question", zh ? "教师追问输入、处理、输出。" : "Instructor asks about input, process, output."], [zh ? "决定" : "Decision", zh ? "共同写下一条修改记录。" : "Write the next revision record together."]])}</div>`, "示范反馈要具体、可执行。", "Model feedback that is specific and actionable."),
      "code-trace": stageSlide("code-trace", `${stageLabel} · REVISION RECORD`, zh ? "每次修改都留下前后记录" : "Record every before-and-after revision", `<pre style="font-size:.55em"><code>${esc('revision = {\n  "issue": "",\n  "evidence_before": "",\n  "change": "",\n  "evidence_after": "",\n  "next_check": ""\n}\nprint(revision)')}</code></pre>`, "修改记录进入Notebook Markdown或反馈表。", "Keep the revision record in notebook Markdown or the feedback form."),
      "guided-practice-1": stageSlide("guided-practice-1", `${stageLabel} · TEACHER FEEDBACK`, zh ? "反馈顺序：证据、问题、一个修改动作" : "Feedback order: evidence, issue, one revision", `<div class="grid3" style="margin-top:.65em">${boxes([[zh ? "证据" : "Evidence", zh ? "先指出已经有效的部分。" : "Identify what already works."], [zh ? "问题" : "Issue", zh ? "引用具体字段、代码或句子。" : "Point to an exact field, line, or sentence."], [zh ? "动作" : "Action", zh ? "只给一个可在本节完成的修改。" : "Give one revision that fits this class."]])}</div>`, "避免一次给出过多反馈。", "Avoid giving too many revisions at once."),
      "guided-practice-2": stageSlide("guided-practice-2", `${stageLabel} · REVISION PLAN`, zh ? "学生把反馈改写成三步计划" : "Student turns feedback into a three-step plan", `<div class="flow" style="margin-top:.75em">${revisions.map((item, i) => `<div class="step${i === 2 ? " loop" : ""}"><span class="n">${i + 1}</span><span>${esc(item)}</span></div>`).join("")}</div>`, "确认计划能在独立操作时间完成。", "Confirm the plan fits the independent-work block."),
      "guided-lab": stageSlide("guided-lab", `${stageLabel} · REVISION TIME`, zh ? "10分钟第一轮修改" : "Ten-minute first revision", `<div class="grid2" style="margin-top:.65em"><div class="box">${list(zh ? ["只修改最高优先级问题", "运行受影响的单元格", "保存修改前后证据"] : ["Revise only the highest-priority issue", "Run the affected cells", "Save before-and-after evidence"])}</div><div class="callout"><b>${zh ? "教师：" : "Instructor: "}</b>${zh ? "巡检，但不代写代码。" : "Check progress without writing the student's code."}</div></div>`, "Stage修改直接在Kaggle Notebook或Draft Paper进行。", "Stage revisions happen directly in the Kaggle notebook or Draft Paper."),
      "mistake-clinic": stageSlide("mistake-clinic", `${stageLabel} · PROJECT RISKS`, zh ? "本阶段最可能导致不通过的风险" : "Risks most likely to block this gate", `<div class="grid3" style="margin-top:.65em">${review.risks.map((item, i) => `<div class="box"><span class="feature">${i + 1}</span><p>${esc(item)}</p></div>`).join("")}</div>`, "风险必须转化为检查动作。", "Turn each risk into a review action."),
      "debug-routine": stageSlide("debug-routine", `${stageLabel} · EXPLAIN-CODE ROUTINE`, zh ? "不能解释时先缩小代码范围" : "When explanation fails, shrink the code scope", `<div class="flow" style="margin-top:.75em"><div class="step"><span class="n">1</span><b>Locate</b><span>${zh ? "选择一行" : "Choose one line"}</span></div><div class="step"><span class="n">2</span><b>Name</b><span>${zh ? "说出变量" : "Name variables"}</span></div><div class="step"><span class="n">3</span><b>Predict</b><span>${zh ? "预测输出" : "Predict output"}</span></div><div class="step loop"><span class="n">4</span><b>Run</b><span>${zh ? "运行并比较" : "Run and compare"}</span></div></div>`, "先恢复理解，再继续项目。", "Restore understanding before continuing the project."),
      checkpoint: stageSlide("checkpoint", `${stageLabel} · RUBRIC PREVIEW`, zh ? "独立操作前先看通过标准" : "Preview the gate rubric before independent work", `<div class="grid2" style="margin-top:.65em">${review.criteria.map((item) => `<div class="box"><p>${esc(item)}</p></div>`).join("")}</div>`, "学生圈出最弱的一项。", "Student identifies the weakest criterion."),
      "project-plan": stageSlide("project-plan", `${stageLabel} · REVISION LOG`, zh ? "写下本节修改记录" : "Write today's revision log", `<pre style="font-size:.55em"><code>${esc('priority = "must fix"\nchange = ""\nevidence = ""\nteacher_feedback = ""\nnext_step = ""')}</code></pre>`, "修改记录是Stage产出的一部分。", "The revision log is part of the Stage output."),
      "project-brief": stageSlide("project-brief", `${stageLabel} · SUBMISSION`, esc(spec.projectTitle), `<div class="grid3" style="margin-top:.65em">${boxes([["Input", spec.projectInput], ["Review", spec.projectProcess], ["Output", spec.projectOutput]])}</div>`, "提交内容必须与Stage目标一致。", "Submission must match the Stage goal."),
      "project-lab": stageSlide("project-lab", `${stageLabel} · SUBMISSION CHECK`, zh ? "逐项打开，而不是只说已经完成" : "Open each artifact; do not merely say it is done", `<div class="grid2" style="margin-top:.65em">${review.materials.map((item) => `<div class="box"><p>□ ${esc(item)}</p></div>`).join("")}</div><div class="callout"><b>${zh ? "链接检查：" : "Link check: "}</b>${zh ? "Notebook和数据链接必须能由教师访问。" : "Notebook and dataset links must be accessible to the instructor."}</div>`, "由学生操作屏幕并展示证据。", "The student controls the screen and presents evidence."),
      "independent-work": stageSlide("independent-work", `${stageLabel} · INDEPENDENT REVISION`, zh ? "20分钟学生独立修改 + 10分钟教师巡检" : "20 minutes independent revision + 10 minutes instructor review", `<div class="grid3" style="margin-top:.65em">${boxes([[zh ? "前10分钟" : "First 10 min", zh ? "完成最高优先级修改。" : "Complete the highest-priority revision."], [zh ? "后10分钟" : "Next 10 min", zh ? "重新运行、核对并记录证据。" : "Rerun, verify, and record evidence."], [zh ? "教师10分钟" : "Instructor 10 min", zh ? "个别反馈后统一纠错。" : "Individual feedback, then one shared correction."]])}</div>`, "教师按名单巡检，保持反馈公平。", "Use a roster for consistent check-ins."),
      "challenge-ladder": stageSlide("challenge-ladder", `${stageLabel} · NEXT GATE`, zh ? "只有当前Stage通过后才进入下一步" : "Advance only after the current gate is met", `<p class="lead">${esc(review.nextGate)}</p><div class="callout"><b>${zh ? "未通过：" : "Not yet: "}</b>${zh ? "列出必须修改项和重新检查时间，不新增高级模型。" : "List required revisions and a recheck time; do not add an advanced model."}</div>`, "把下一阶段入口写成明确条件。", "Express the next-stage entry as an explicit condition."),
      rubric: stageSlide("rubric", `${stageLabel} · STAGE RUBRIC`, zh ? "本阶段评价以证据和解释为中心" : "This gate evaluates evidence and explanation", `<div class="grid2" style="margin-top:.65em">${review.criteria.map((item, i) => `<div class="box"><h3>${i + 1}. ${esc(item)}</h3><p>${zh ? "未满足 / 需要修改 / 已通过" : "Not met / Revise / Passed"}</p></div>`).join("")}</div>`, "Rubric结果与修改记录一起保存。", "Save the rubric result with the revision record."),
    };
    return S.map((html) => {
      const match = html.match(/^<section id="([^"]+)"/);
      return match && overrides[match[1]] ? overrides[match[1]] : html;
    });
  }
  return S;
}
...
function buildLiveEnglishClassOne(spec) {
  const narrationDisabledIds = new Set([
    "prerequisite-recap", "one-twenty-minute-pacing", "course-position", "warmup",
    "vocabulary", "mental-model", "concept-3",
  ]);
  const slidesById = new Map(buildKaggleLesson(spec, "en").map((html) => {
    const match = html.match(/^<section id="([^"]+)"/);
    return [match ? match[1] : "", html];
  }));
  return liveEnglishClassOneSlideIds.map((id) => {
    const html = slidesById.get(id);
    return html && narrationDisabledIds.has(id)
      ? html.replace(/^<section id="([^"]+)"/, '<section id="$1" data-narration="off"')
      : html;
  }).filter(Boolean);
}
...
const liveCatalog = (lessons, lang) => Object.values(lessons)
  ...
  .map((lesson) => ({
    ...
    duration: lang === "zh" ? `${lesson.duration} 分钟` : `${lesson.duration} minutes`,
    status: "ready",
  }));
...
Object.keys(syncedZhLessons).forEach((no) => {
  ...
  DECK_BUILDERS[`python-ai/${lessonId}`] = {
    build: () => ["01", "02"].includes(no)
      ? buildKaggleLesson(syncedZhLessons[no], "zh")
      : buildSyllabusLesson(syncedZhLessons[no], "zh"),
