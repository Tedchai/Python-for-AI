...
function buildKaggleLesson(spec, lang) {
  ...
  if (spec.classNo === "01") {
    S.push(sec("prerequisite-recap", `<div class="kicker">${zh ? "第一课 · 从这里开始" : "CLASS 01 · START HERE"}</div><h2>${zh ? "第一次见面：零基础也能完成一份真实的 Python 成果" : "First meeting: complete a real Python result with no prior knowledge"}</h2><div class="grid2" style="margin-top:.55em"><div class="box"><h3>${zh ? "不需要提前学过" : "No prior experience required"}</h3><p>${zh ? "不要求学过 Python 或 Kaggle；只需要打开浏览器，跟着教师亲手运行和修改代码。" : "No Python or Kaggle background is required; open a browser and run and modify code with the instructor."}</p></div><div class="box"><h3>${zh ? "今天当堂带走" : "Leave with a real result"}</h3><p>${esc(spec.output)}</p></div></div>${!zh ? `<div class="flow" style="margin-top:.55em"><div class="step"><span class="n">1</span><b>Game logic</b><span>A quick interest hook</span></div><div class="step"><span class="n">2</span><b>Data stories</b><span>Charts that support a claim</span></div><div class="step"><span class="n">3</span><b>AI experiments</b><span>Models and fair comparisons</span></div><div class="step loop"><span class="n">4</span><b>Research poster</b><span>The main communication goal</span></div></div>` : ""}<div class="callout"><b>${zh ? "课堂方式：" : "How class works: "}</b>${zh ? "教师先示范，学生现场运行、修改并保存自己的成果；每一步都看得见、说得清。" : "The instructor shows the workflow first; students then run, modify, and save their own work, with every step visible and explainable."}</div>${notes("第一节课先降低进入门槛并建立信心，不使用复习或续课话术。", "Use game design only as one brief example of what Python can create. Keep the course destination focused on evidence, AI research, and a professional research poster.")}`));
  } else {
