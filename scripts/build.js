...
function buildKaggleLesson(spec, lang) {
  ...
  if (spec.classNo === "01") {
    S.push(sec("rubric", `<div class="kicker">TODAY'S SUCCESS CHECK</div><h2>${zh ? "完成第一课，要留下四个可见结果" : "Complete Class 01 with four visible results"}</h2><div class="grid2" style="grid-template-columns:1fr 1fr;margin-top:.65em"><div class="box"><h3>Run</h3><p>${zh ? "亲手运行了至少一段Python代码。" : "Run at least one Python code block personally."}</p></div><div class="box"><h3>Change</h3><p>${zh ? "修改了一个变量并看到输出变化。" : "Change one variable and observe the output."}</p></div><div class="box"><h3>Explain</h3><p>${zh ? "能指出一项输入和一项输出。" : "Identify one input and one output."}</p></div><div class="box"><h3>Save</h3><p>${zh ? "保存了自己的Notebook副本。" : "Save a personal notebook copy."}</p></div></div>${notes("第一课不强调分数，强调学生亲手完成的证据。", "Class 01 emphasizes evidence of doing rather than grades.")}`));
    S.push(sec("homework", `<div class="kicker">KEEP YOUR RESULT</div><h2>${zh ? "下课前，把今天的成果真正留在自己手里" : "Keep today's result before class ends"}</h2><div class="grid2" style="margin-top:.65em"><div class="box">${list(spec.homework)}</div><div class="box"><h3>${zh ? "离开前确认" : "Before leaving"}</h3>${list(zh ? ["代码从上到下可运行", "四种类型至少出现三种", "有一句自己的修改", "Notebook链接可以访问"] : ["Code runs top to bottom", "At least three of the four types appear", "One change is personal", "The notebook link is accessible"])}</div></div>${notes("不布置新的大任务，只确保学生带走可运行成果。", "Do not assign a new large task; make sure the student leaves with a runnable artifact.")}`));
  } else {
